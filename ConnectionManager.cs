...
public class ConnectionManager : IDisposable
{
    ...
    protected virtual void Dispose(bool disposing)
    {
        ...
        foreach (var (key, con) in _connections)
            ...
            {
                Console.WriteLine("Exception thrown while disposing connection (TcpRpcClient): " + key +
                                  " Exception: " + e.Message);
            }
        ...
        where TRemoteInterface : class, IDisposable
    {
        ...
        else
        {
            ...
            if (vatIdAndRest[^1].Contains('/'))
            {
                ...
                if (addressPortAndRest.Length > 0)
                {
                    ...
                    if (rawHostPort.Length > 1) port = int.Parse(rawHostPort[1]);
                }
        ...
        while (retryCount > 0)
        {
            try
            {
                ...
                if (attemptTls) con = await TryTlsConnectAsync(host, connectHost, port);
