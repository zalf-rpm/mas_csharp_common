...
{
    private readonly ConcurrentDictionary<string, ((ulong, ulong, ulong, ulong), string)>
    ...
    private readonly ConcurrentDictionary<(ulong, ulong, ulong, ulong), P.IRestorer> _vatId2Restorer = new();
    ...
    public async Task<BareProxy> Restore(P.Restorer.RestoreParams ps,
        CancellationToken cancellationToken_ = default)
    {
        ...
        if (_extSRT2VatIdAndIntSRT.TryGetValue(srToken, out var value))
        {
            ...
            if (_vatId2Restorer.TryGetValue(vatId, out var restorer))
                return await restorer.Restore(new P.Restorer.RestoreParams
                {
        ...
        if (cap is BareProxy bareProxy) return Proxy.Share(bareProxy);
    ...
    public (string, string) SaveStr(Proxy proxy, string fixedSrToken = null,
    ...
    public static string SturdyRefStr(P.SturdyRef sturdyRef)
    {
        ...
        return
            $"capnp://{vatIdBase64Url}@{sturdyRef.Vat.Address.Host}:{sturdyRef.Vat.Address.Port}/{sturdyRef.LocalRef.Text}";
    }
    ...
    public SaveRes Save(Proxy proxy, string fixedSRToken = null, string sealForOwner = null,
